defmodule Momento.Add do
  ...
  def add(%DateTime{} = datetime, num, :months)
      when positive?(num) and num > 11 do
    years = floor(num / 12)
  ...
  def add(%DateTime{month: month, day: day} = datetime, num, :days)
      ...
      do:
        add(%DateTime{datetime | day: 1}, 1, :months)
        |> add(num - (days_in_month(month) - day) - 1, :days)
  ...
  def add(%DateTime{} = datetime, num, :hours)
      when positive?(num) and num > 24 do
    days = floor(num / 24)
  ...
  def add(%DateTime{} = datetime, num, :minutes)
      when positive?(num) and num > 60 do
    hours = floor(num / 60)
  ...
  def add(%DateTime{minute: minute} = datetime, num, :minutes)
      ...
      do:
        add(%DateTime{datetime | minute: 0}, 1, :hours) |> add(num - (60 - minute) - 1, :minutes)
  ...
  def add(%DateTime{second: second} = datetime, num, :seconds)
      ...
      do:
        add(%DateTime{datetime | second: 0}, 1, :minutes)
        |> add(num - (60 - second) - 1, :seconds)
  ...
  def add(%DateTime{microsecond: {microsecond, precision}} = datetime, num, :milliseconds)
      ...
      do: %DateTime{
        datetime
        | microsecond: {microsecond + num * millisecond_factor(precision), precision}
      }

  # Many seconds worth of milliseconds
  def add(%DateTime{microsecond: {_, precision}} = datetime, num, :milliseconds)
      when positive?(num) and num > 999 and precision >= 3 do
    seconds = Float.floor(num / millisecond_factor(precision)) |> round

    add(datetime, seconds, :seconds)
    |> add(num - seconds * millisecond_factor(precision), :milliseconds)
  end
  ...
  def add(%DateTime{microsecond: {microsecond, precision}} = datetime, num, :microseconds)
      when natural?(num) and precision === 6 and microsecond + num <= 999_999,
  ...
  def add(%DateTime{microsecond: {_, precision}} = datetime, num, :microseconds)
      when positive?(num) and precision === 6 and num > 999_999 do
    seconds = Float.floor(num / microsecond_factor(precision)) |> round

    add(datetime, seconds, :seconds)
    |> add(num - seconds * microsecond_factor(precision), :microseconds)
  end
end
